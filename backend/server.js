...
async function searchItems(request, response) {
    //begin here
    var searchField = request.query.taskname; // getting the search field from the query parameter
    var json = JSON.parse(await fsPromises.readFile("database.json")); // reading the json file
    var returnData = json.filter(jsondata => jsondata.Task === searchField); // getting data based on the search field
    response.json(returnData); // returning the data
};
